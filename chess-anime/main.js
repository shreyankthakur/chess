...
function updateCharacterUI(side, label) {
  ...
  const selectName = document.getElementById(`select-${side}-name`);
  if (cardName) cardName.textContent = label;
  if (selectName) selectName.textContent = label;
}
...
function refreshOnlinePanel() {
  ...
  onlineStatus.textContent = onlineMode === 'pass'
    ...
    : 'Online lobby ready — no server connected';
  ...
  if (onlineMode === 'online') {
    fakeFriends.forEach(name => {
      ...
      pill.textContent = `${name} • online`;
      friendList.appendChild(pill);
    });
  } else {
    const pill = document.createElement('div');
    pill.className = 'online-pill';
    pill.textContent = 'No friends yet';
    friendList.appendChild(pill);
  }

  challengeList.innerHTML = '';
  if (fakeChallenges.length) {
    fakeChallenges.forEach(challenge => {
      const pill = document.createElement('div');
      pill.className = 'online-pill';
      pill.textContent = `Challenge: ${challenge.from} → ${challenge.type}`;
      challengeList.appendChild(pill);
    });
  } else {
    const pill = document.createElement('div');
    pill.className = 'online-pill';
    pill.textContent = onlineMode === 'online'
...
if (gameModeSelect) {
  gameModeSelect.addEventListener('change', (e) => {
    ...
    if (onlineMode === 'online') {
      onlineStatus.textContent = 'Online lobby active — simulate server connect';
      fakeChallenges = ['NovaKnight challenge'];
    } else {
